...
export async function saveDraft(chatId, result) {
  const id = newId();
  const record = { id, chatId, status: 'awaiting_review', version: 1, history: [], createdAt: new Date().toISOString(), ...result };
  if (backend === 'supabase') {
    await rest('drafts', {
      method: 'POST',
      prefer: 'return=minimal',
      body: { id, chat_id: String(chatId), status: record.status, score: result.screen.score, core_idea: result.screen.core_idea, record },
    });
  } else {
...
export async function updateDraft(chatId, record) {
  if (backend === 'supabase') {
    await rest(`drafts?id=eq.${encodeURIComponent(record.id)}&chat_id=eq.${encodeURIComponent(chatId)}`, {
      method: 'PATCH',
      prefer: 'return=minimal',
      body: { status: record.status, score: record.screen.score, core_idea: record.screen.core_idea, record },
    });
  } else {
    const { writeFileSync } = await import('node:fs');
    writeFileSync(`.local-drafts/${record.createdAt.slice(0, 10)}-${record.id}.json`, JSON.stringify(record, null, 2) + '\n');
  }
}

export async function deleteDraft(chatId, id) {
  if (backend === 'supabase') {
    await rest(`drafts?id=eq.${encodeURIComponent(id)}&chat_id=eq.${encodeURIComponent(chatId)}`, { method: 'DELETE' });
  } else {
    const record = await getDraft(chatId, id);
    if (record) (await import('node:fs')).rmSync(`.local-drafts/${record.createdAt.slice(0, 10)}-${id}.json`);
  }
}
