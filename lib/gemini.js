...
async function withFallback(primary, fn, { singleTry = false } = {}) {
  ...
  for (let pass = 1; pass <= 2; pass++) {
    for (const model of models) {
      for (let attempt = 1; attempt <= 2; attempt++) {
        ...
        try {
          return await fn(model, Math.min(45_000, left - 2_000));
        } catch (err) {
          lastErr = err;
          ...
          console.error(`gemini ${model} ${isAbort(err) ? 'timed out' : /PerDay/i.test(msg) ? 'daily quota used up' : err.status}, trying next model`);
          break;
        }
      }
    }
...
export async function generateJSON({ model, system, prompt, schema, temperature = 0.4, thinking }) {
