...
import { generateJSON, generateGrounded, withBudget, SCREEN_MODEL, DRAFT_MODEL } from './gemini.js';
import { env } from './env.js';
import { findNewsAngle, newsEnabled } from './news.js';
import { lintVoice, unsupportedNumbers, parseAngle, sourcesForFact } from './checks.js';

export const PASS_SCORE = Number(env('PASS_SCORE') || 6);
...
// Google News + Claude when an Anthropic key is set; otherwise Gemini with Google Search grounding.
export async function findAngle(screen) {
  if (newsEnabled()) return withBudget(35_000, () => findNewsAngle(screen));
  return findGroundedAngle(screen);
}

async function findGroundedAngle(screen) {
...
RELEVANCE: <one sentence on how it connects to the post idea>`;
  ...
  // The angle is optional, so it gets a short slice of the budget and never starves the draft.
  const { text, chunks, supports, queries } = await withBudget(35_000, () => generateGrounded({ model: SCREEN_MODEL, system, prompt }));
...
const DRAFT_SCHEMA = {
  ...
  properties: {
    ...
    facts_to_verify: { type: 'array', items: { type: 'string' }, description: 'Every factual claim in the post that Meera should confirm, especially anything phrased more specifically or strongly than in her note.' },
  },
...
- This applies to details written in words too: no added timeframes ("another year"), quantities ("several", "most brands"), outcomes ("rendering it ineffective"), or test conditions that the note doesn't state. Keep her claims exactly as strong as she made them — "a couple of samples" stays a couple.
...
function draftPrompt(note, screen, angle, previous = []) {
  const angleBlock = angle?.usable
    ? `Sourced angle (from ${angle.provider === 'google-news' ? 'a Google News result; FACT is quoted from its headline or snippet' : 'web search'}${angle.verified ? '' : ' — not tied to a specific result, treat cautiously'}):
${angle.headline ? `HEADLINE: ${angle.headline}\n` : ''}FACT: ${angle.fact}
SOURCE: ${angle.source}
DATE: ${angle.date}
...
${angleBlock}
${previous.length ? `
Meera asked for a redraft. Earlier versions she didn't approve:
${previous.map((p, i) => `--- version ${i + 1} ---\n${p}`).join('\n')}

Write a genuinely different version: change the opening (number, scene, or plain claim) and the order of the argument, not just the wording. Every rule above still applies.
` : ''}
Write the post.`;
}

export async function writeDraft(note, screen, angle, ctx = loadContext(), previous = []) {
  return generateJSON({
    model: DRAFT_MODEL,
    system: draftSystem(ctx),
    prompt: draftPrompt(note, screen, angle, previous),
    label: 'draft',
    schema: DRAFT_SCHEMA,
    temperature: 0.7,
  });
...
export async function runPipeline(note, { onStage } = {}) {
  ...
  onStage?.('drafting');
  return { status: 'drafted', note, screen, angle, ...(await draftFromScreen(note, screen, angle, { ctx })) };
}

// Drafting on its own, reused by Redraft: same note, screening and angle; earlier versions steer it elsewhere.
export async function draftFromScreen(note, screen, angle, { ctx = loadContext(), previous = [] } = {}) {
  let draft = await writeDraft(note, screen, angle, ctx, previous);
  ...
  return { draft, voiceIssues, unverifiedNumbers };
}
