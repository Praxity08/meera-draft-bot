...
export async function handleUpdate(update) {
  ...
  const [ack] = await sendMessage(chatId, 'Working on this note. A draft usually takes about a minute. If nothing appears within 3 minutes, send the note again.', reply);
  await sendTyping(chatId);

  let result;
  try {
    result = await withBudget(BUDGET_MS, () => runPipeline(text, { onStage: () => sendTyping(chatId) }));
  } catch (err) {
  ...
  return `drafted ${record.id} (${result.screen.score}/10, angle ${result.angle.usable ? 'used' : result.angle.error ? 'failed' : 'none'})`;
}
