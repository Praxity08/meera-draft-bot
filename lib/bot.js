...
async function sendDraft(chatId, record) {
  const extra = replyTo(record.messages?.note);
  const review = await sendMessage(chatId, formatReview(record, record.id), extra);
  ...
  switch (action) {
    ...
    case 'r': {
      ...
      try {
        ...
        const next = await draftFromScreen(record.note, record.screen, angle, { previous });
