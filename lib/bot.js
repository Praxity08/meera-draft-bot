...
import { isTransient, withBudget, currentTrace } from './gemini.js';
...
export async function handleUpdate(update) {
  ...
  let result;
  let trace = [];
  const audioInfo = audio ? ` [voice ${audio.duration ?? '?'}s ${audio.mimeType} ${audio.size ?? '?'}B]` : '';
  try {
  ...
  return `drafted ${record.id}${audioInfo} (${result.screen.score}/10, angle ${result.angle.usable ? 'used' : result.angle.error ? 'failed' : 'none'})\n${trace.join('\n')}`;
}
