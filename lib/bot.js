...
export async function handleUpdate(update) {
  ...
  await sendMessage(chatId, 'Got it. Screening this note.');
  await sendTyping(chatId);

  try {
    const result = await runPipeline(text, { onStage: () => sendTyping(chatId) });
    if (result.status === 'rejected') return sendMessage(chatId, formatRejection(result));
    ...
    await sendMessage(chatId, formatReview(result, id));
    await sendMessage(chatId, result.draft.post);
  } catch (err) {
    console.error('pipeline failed:', err);
    await sendMessage(chatId, `Something failed while processing that note (${err?.message ?? 'unknown error'}). Nothing was saved. Try sending it again.`);
  }
}
